const Register = () => {
    return <>
        <section className=" text-center p-10 tracking-wider">
            
            <button className="rounded-full text-white font-bold bg-red-600 p-5 text-2xl m-5">Inscribase y participe <img src="https://res.cloudinary.com/dyhftwfrw/image/upload/v1708540321/davi/lnnauduleneuihmbrqjd.svg" alt="Inscribase y participe" className="img-button"/></button>
            <p className="text-center text-black text-2xl">
                Mientras tanto, <span className="font-bold">vaya por las fotocopias o las onces, </span><span className=" text-red-600 italic font-bold underline underline-offset-8">descangando su DaviPlata</span>
            </p>
        </section>
    </>
}
export default Register